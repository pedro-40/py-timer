...
def ctext(color:str, text:str) -> str:
    return color+f"{text}"+Fore.RESET


def get_start_question() -> str:
    return input(ctext(Fore.CYAN, "Start The Timer? (Y/n): "))


def get_time() -> tuple[str]:
    t = tuple(map(
            lambda x: int(x.strip()),
            input(ctext(Fore.GREEN, "Enter Time in HH:MM:SS format : ")).split(":")
        ))
    if len(t) != 3:
        print(ctext(Fore.RED, "Entered Time is invalid\nExample valid time : 12:36:00"))
        return get_time()
